...
import ru.kulkov.commands.RotatingObjectAdaptor;
...
public class Application {
    public static void main(String[] args) {
        ...
        ship.setProperty("angle", new Angle(90));
        ship.setProperty("angularVelocity", new Angle(45));

        Move moveOperation = new Move(new MovingObjectAdaptor(ship));
        moveOperation.execute();

        Rotate rotateOperation = new Rotate(new RotatingObjectAdaptor(ship));
        ...
        Angle newAngle = ship.getProperty("angle", Angle.class);
        System.out.println("New angle: " + newAngle.getDegrees());
    }
}
